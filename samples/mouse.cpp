...
//     * printed by corresponding functions
//     */
//    auto geoPose = geoMap()->getProjection()->projToGeo(projPos);
//    mText->setText(QString("<b>%1, %2</b>").arg(geoPose.latToString()).arg(geoPose.lonToString()));
//}
 //void QGVMap::mousePressEventw(QMouseEvent* event)
//{
//    if (hasMouseTracking()) {
//        Q_EMIT mapMouseMove(mapToProj(event->pos()));
//    }

//    //auto geoPos = geoView()->getProjection()->projToGeo(projPos);
//    //auto geoPos = getProjection()->projToGeo(mapToProj(event->pos()));
//    //QPointF coords = geoView()->mapFromParent(event->pos());
//    //qDebug() << "Latitude:" << coords.y() << "Longitude:" << coords.x();
//   // qDebug() << "Latitude:" << geoPos.latToString() << "Longitude:" << geoPos.lonToString();
